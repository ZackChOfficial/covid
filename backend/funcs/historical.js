...
var historical = async (keys, redis) => {
  ...
  try {
    casesResponse = await axios.get(
      `https://raw.githubusercontent.com/brahimElbouziani/zack/master/time_series_covid19_confirmed_global.csv`
    );
  ...
  for (let b = 0; b < parsedCases.length; ) {
    const timeline = {
      cases: {}
    };
  ...
  console.log(
    `Updated JHU CSSE Historical: ${removeFirstObj.length} locations`
  );
};

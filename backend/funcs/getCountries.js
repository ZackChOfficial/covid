...
var getcountries = async (keys, redis) => {
  ...
  for (let i = 0; i < countriesTableCells.length - totalColumns; i += 1) {
    ...
    if (i % totalColumns === countryColIndex) {
      ...
      result.push({
        country,
        countryInfo: countryUtils.getCountryData(country)
      });
    }
    // get cases
    ...
    if (i % totalColumns === todayCasesColIndex) {
      ...
      result[result.length - 1].todayCases = parseInt(
        cases ? cases.trim().replace(/,/g, "") : "0" || "0",
        10
      );
    }
    // get deaths
    if (i % totalColumns === deathsColIndex) {
      ...
      result[result.length - 1].deaths = parseInt(
        deaths ? deaths.trim().replace(/,/g, "") : "0" || "0",
        10
      );
    }
    // get today deaths
    ...
    if (i % totalColumns === curedColIndex) {
      ...
      result[result.length - 1].recovered = parseInt(
        cured ? cured.trim().replace(/,/g, "") : 0 || 0,
        10
      );
    }
    // get active
    ...
    if (i % totalColumns === criticalColIndex) {
      ...
      result[result.length - 1].critical = parseInt(
        critical ? critical.trim().replace(/,/g, "") : "0" || "0",
        10
      );
    }
    ...
    if (i % totalColumns === casesPerOneMillionColIndex) {
      ...
      result[result.length - 1].casesPerOneMillion = parseInt(
        casesPerOneMillion
          ? casesPerOneMillion.trim().replace(/,/g, "")
          : "0" || "0",
        10
      );
    }
